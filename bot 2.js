...
async function fetchTopTraders() {
  ...
  const PATHS = [
    '/profiles?limit=50&sortBy=profitLoss&sortDirection=DESC',
    '/profiles?limit=50&sortBy=volume&sortDirection=DESC',
    '/leaderboard?window=all&limit=50',
  ];
...
async function getPositions(address) {
  try {
    ...
    const path = `/trades?user=${address}&limit=20&sortBy=timestamp&sortDirection=DESC`;
    ...
    if (!trades.length) {
      ...
      const path2 = `/positions?user_address=${address}&size_threshold=0.01`;
