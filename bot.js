// POLYBOT — Railway Server v9.2 — endDate guard on resolution (no more phantom wins)
...
async function resolveOpenTrades(markets) {
  ...
  for (const trade of openTrades) {
    ...
    // A market is ONLY resolved when Polymarket closes it AND the endDate has passed
    // Prices snap to exactly 0.0 or 1.0 on resolution
    // A price of 0.001 (0.1%) is still an ACTIVE open market — NOT resolved
    // CRITICAL: A noPrice of 0.999 on a future-dated market is NOT a resolution —
    // it just means the market is heavily one-sided but still open. We must check endDate.
    const marketEndTs = mkt.endDate ? new Date(mkt.endDate).getTime() : Infinity;
    const marketHasEnded = Date.now() > marketEndTs;
    const yesWon = marketHasEnded && mkt.yesPrice >= 0.999;  // YES resolved: price = 1.0
    const noWon  = marketHasEnded && mkt.noPrice  >= 0.999;  // NO resolved:  price = 1.0
    ...
    if (won) {
      PERSIST.wins++;
      // Real profit: you paid entryPrice per share, collect $1.00 per share
      // profit = size_in_usdc * (1 - entryPrice) / entryPrice
