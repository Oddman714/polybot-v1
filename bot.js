// ═══════════════════════════════════════════════════
// POLYBOT — Railway Server v4 — FINAL
...
const CFG = {
  ...
  claudeEdge:      parseFloat(process.env.CLAUDE_EDGE)      || 0.06,  // 6% edge = more signals
...
async function fetchMarkets() {
  try {
    const data = await httpsGet(
      'https://gamma-api.polymarket.com/markets?closed=false&limit=40&sort=volume&order=desc',
      { 'User-Agent': 'Polybot/3.0' }
    );
...
async function callClaude(systemPrompt, userPrompt) {
  if (!CFG.claudeKey) {
    logError('callClaude', new Error('CLAUDE_API_KEY not set'));
    return null;
  }
...
async function claudeAnalyzeMarkets(markets, headlines) {
  ...
  const liquid = markets
    .filter(m => parseFloat(m.volume || m.volumeNum || 0) >= CFG.minVolume)
    .slice(0, 15); // analyze more markets for more signals

  if (!liquid.length) {
    log('No liquid markets to analyze (all below min volume)');
    return [];
  }

  const marketSummary = liquid.map((m, i) => {
    const price = parseFloat(m.bestAsk || m.lastTradePrice || 0.5);
...
For each market, estimate the TRUE probability of YES resolving based on the news and your knowledge. Only flag markets where your estimate differs from the market price by ${(CFG.claudeEdge * 100).toFixed(0)}% or more — those are our betting opportunities.
...
Only include markets with edge >= ${CFG.claudeEdge}. If none qualify, return {"signals":[],"summary":"no edge found"}`;
...
async function executeClaudeSignals(signals, markets) {
  ...
  for (const signal of signals) {
    ...
    if (signal.edge < CFG.claudeEdge) continue;
...
async function scanArb(markets) {
  for (const mkt of markets.slice(0, 20)) {
    try {
      // Try multiple price fields — Polymarket API returns different shapes
      ...
      const yes = Math.min(0.99, Math.max(0.01, yesPrice));
      const no  = Math.min(0.99, Math.max(0.01, noPrice));
      const sum = yes + no;
...
async function poll() {
  ...
  try {
    ...
    if (STATE.claudePollCount % CFG.claudePollEvery === 0 && CFG.claudeKey) {
...
app.get('/', (req, res) => {
  ...
  res.sendFile(htmlPath, (err) => {
    if (err) res.json({ ok: true, uptime: Date.now() - STATE.startedAt, version: '4.0' });
  });
});

// Health check JSON
app.get('/health', (req, res) => res.json({ ok: true, uptime: Date.now() - STATE.startedAt, version: '4.0' }));
