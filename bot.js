...
async function fetchMarkets() {
  try {
    ...
    const data = await httpsGet(
      ...
      { 'User-Agent': 'Polybot/5.4' }
    );
    let markets = Array.isArray(data) ? data : (data?.markets || []);

    // Filter to active markets with real volume
    markets = markets.filter(m => {
      const vol = parseFloat(m.volume || m.volumeNum || m.liquidity || 0);
      const active = !m.closed && !m.archived && !m.resolved;
      return vol >= 1000 && active;
    });
...
app.get('/', (req, res) => {
  ...
  res.sendFile(htmlPath, (err) => {
    if (err) res.json({ ok: true, uptime: Date.now() - STATE.startedAt, version: '5.5' });
  });
});

// Health check JSON
app.get('/health', (req, res) => res.json({ ok: true, uptime: Date.now() - STATE.startedAt, version: '5.5' }));
