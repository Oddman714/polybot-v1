// POLYBOT — Railway Server v8.4 — WS fix + throttling + hardened state
...
const CFG = {
  ...
  // Real copy trader wallet addresses — set in Railway Variables
  // Format: COPY_WALLETS=0xABC123,0xDEF456,0xGHI789
    ...
    try {
      ...
      for (const trade of trades.slice(0, 5)) {
        ...
        PERSIST.trades.push({
          ...
          price: price,  // store actual entry price for accurate loss calc
          trader: traderName,
        });
        ...
        // TODO: Live CLOB order signing with POLY_PRIVATE_KEY goes here
      }
    } catch (e) {
...
{"signals":[{"market":"exact question text","outcome":"YES or NO","confidence":"high/medium/low","edge":0.09,"size":${(PERSIST.bankroll*0.08).toFixed(2)},"reasoning":"specific data-backed reason citing orderbook/flow/news"}],"summary":"1-line market overview"}`;
  ...
  try {
    ...
    for (const sig of signals) {
      ...
      const sigPrice = markets.find(m => m.question === sig.market)?.yesPrice ||
                       (sig.outcome === 'NO' ? 0.01 : 0.99);
...
function resolveOpenTrades() {
  ...
  // Resolve trades older than 2 minutes (simulate fast market resolution)
  let changed = false;
  for (const trade of openTrades) {
    const age = now - (trade.ts || 0);
    if (age < 30000) continue; // wait 30 sec before resolving in paper mode
    
    // Simulate resolution based on edge confidence
    // Higher edge = higher win probability
    const edge = trade.edge || 0.06;
    // Weight win probability by trade type
    // Copy trades from verified profitable wallets have their real win rates
    let baseWinProb = 0.54 + (edge * 1.5);
    if (trade.type === 'copy') {
      // nicoco89 = 77% WR, classified = 72% WR — use 74% average
      baseWinProb = 0.74;
    } else if (trade.type === 'claude') {
      // Claude signals with high confidence have real edge
      baseWinProb = 0.52 + (edge * 2);
    }
    const won = Math.random() < Math.min(baseWinProb, 0.82);
    ...
    if (won) {
      PERSIST.wins++;
      const tradePrice = parseFloat(trade.price) || (trade.edge ? Math.max(0.001, 1 - trade.edge) : 0.5);
    const profit = Math.min(trade.size * (1 - tradePrice) / Math.max(tradePrice, 0.001), trade.size * 50);
      addFeed({
        ts: now, type: trade.type || 'claude', kind: 'model',
        market: trade.market,
      ...
      log(`[RESOLVED WIN] "${trade.market?.slice(0,40)}" +$${profit.toFixed(2)}`);
    } else {
      PERSIST.losses++;
      addFeed({
        ts: now, type: trade.type || 'claude', kind: 'model',
      ...
      log(`[RESOLVED LOSS] "${trade.market?.slice(0,40)}" -$${trade.size.toFixed(2)}`);
    }
    changed = true;
  }
...
// On Polymarket: buy YES/NO at price p with size s USDC
//   Win:  profit = s * (1 - p) / p   [you get back 1 USDC per share, paid p]
//   Loss: cost   = s               [you lose your stake]
// Cap win at 50x to prevent unrealistic outliers in paper mode
function calcPnl(trades) {
  return trades
    .filter(t => t.status === 'win' || t.status === 'loss')
    .reduce((sum, t) => {
      const size  = parseFloat(t.size) || 0;
      const price = parseFloat(t.price) || (t.edge ? Math.max(0.001, 1 - t.edge) : 0.5);
      if (t.status === 'win') {
        const profit = size * (1 - price) / Math.max(price, 0.001);
        return sum + Math.min(profit, size * 50); // cap at 50x
      } else {
        return sum - size; // lose stake
      }
    }, 0);
}
