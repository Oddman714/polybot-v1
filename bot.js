...
const STATE = {
  ...
  claudePollCount: 0,
  modelSignals: [],
  claudeSignalsToday: 0,
  claudeLastSignalTs: null,
...
async function executeClaudeSignals(signals, markets) {
  ...
  for (const signal of signals) {
    ...
    // Dedup check — expire after 5 minutes so Claude can re-signal
    const posKey = `claude:${mktId}:${signal.side}`;
    const existing = STATE.openPositions[posKey];
    if (existing && Date.now() - existing.ts < 300000) {
      log('Claude signal still open (< 5min)', { posKey });
      continue;
    }
    ...
    STATE.claudeSignalsToday = (STATE.claudeSignalsToday || 0) + 1;
    STATE.claudeLastSignalTs = Date.now();
    ...
    // Show resolution in feed when paper trade resolves
    if (result?.pnl !== undefined) {
      const win = result.pnl > 0;
      pushFeed({
        kind: 'model',
        icon: win ? '✅' : '❌',
        cls:  win ? 'ic-win' : 'ic-loss',
        mkt:  signal.market,
        det:  `[PAPER] ARB ${signal.side}@${signal.marketPrice}`,
        amt:  (win ? '+' : '') + '$' + Math.abs(result.pnl).toFixed(2),
        ac:   win ? 'pos' : 'neg',
        win:  win,
      });
    }
...
app.get('/', (req, res) => {
  ...
  res.sendFile(htmlPath, (err) => {
    if (err) res.json({ ok: true, uptime: Date.now() - STATE.startedAt, version: '5.8' });
  });
});

// Health check JSON
app.get('/health', (req, res) => res.json({ ok: true, uptime: Date.now() - STATE.startedAt, version: '5.8' }));
...
app.get('/state', (req, res) => {
  ...
  res.json({
    ...
    claudeActive: !!(CFG.claudeKey && STATE.claudePollCount > 0),
    lastClaudeResponse: STATE.lastClaudeResponse || 'not called yet',
    claudePollCount: STATE.claudePollCount,
    claudeSignalsToday: STATE.claudeSignalsToday || 0,
    claudeLastSignalTs: STATE.claudeLastSignalTs || null,
