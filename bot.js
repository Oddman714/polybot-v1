// POLYBOT — Railway Server v9.3 — LIVE EXECUTION + market quality filter + signal tracking
  ...
  for (const traderAddr of PERSIST.selectedTraders) {
    try {
      ...
      for (const trade of trades.slice(0, 5)) {
        ...
        const tradeKey = `${traderAddr.slice(0,10)}-${mktId}-${outcome}-${Math.round(price*1000)}`;
...
// Initializes once, reuses across orders
let _clobClient = null;

async function getClobClient() {
  if (_clobClient) return _clobClient;
  if (!CFG.polyPrivateKey || !ethers || !ClobClient) {
    log('CLOB client unavailable — missing ethers, clob-client, or POLY_PRIVATE_KEY');
    return null;
  }
  try {
    const wallet = new ethers.Wallet(CFG.polyPrivateKey);
    const creds = CFG.polyApiKey ? {
      key:        CFG.polyApiKey,
      secret:     CFG.polySecret,
      passphrase: CFG.polyPassphrase,
    } : null;
    ...
    _clobClient = new ClobClient(
      ...
      wallet,
      creds,
      1,   // signature_type=1 for proxy/Magic wallets
      CFG.polyWalletAddr || wallet.address
    );

    // If no creds yet, derive them now and save to persist
    if (!creds) {
      log('Deriving CLOB API credentials from private key...');
      const derived = await _clobClient.createOrDeriveApiCreds();
      PERSIST.polyApiKey       = derived.key        || derived.api_key;
      PERSIST.polySecret       = derived.secret      || derived.api_secret;
      PERSIST.polyPassphrase   = derived.passphrase  || derived.api_passphrase;
      savePersist();
      log('CLOB credentials derived and saved', {
        key: (PERSIST.polyApiKey || '').slice(0, 8) + '...'
      });
      // Reinit with creds
      _clobClient = new ClobClient(
        'https://clob.polymarket.com', 137, wallet,
        { key: PERSIST.polyApiKey, secret: PERSIST.polySecret, passphrase: PERSIST.polyPassphrase },
        1, CFG.polyWalletAddr || wallet.address
      );
    }
    log('CLOB client initialized', { address: (CFG.polyWalletAddr || wallet.address).slice(0,12) });
    return _clobClient;
  } catch(e) {
    logError('getClobClient', e);
    return null;
  }
}
...
async function executeCLOBOrder(tokenId, price, sizeUSDC, side = 'BUY') {
  try {
    ...
    // Validate tick size — Polymarket requires price at valid ticks
    const tickSize = price < 0.1 ? 0.001 : price < 0.5 ? 0.01 : 0.01;
    const roundedPrice = Math.round(price / tickSize) * tickSize;
    const minSize = 1.0; // Polymarket minimum order $1 USDC

    if (sizeUSDC < minSize) {
      log(`Order too small: $${sizeUSDC.toFixed(2)} < $${minSize} minimum`);
      return null;
    }

    const order = await client.createAndPostOrder(
      { tokenID: tokenId, price: roundedPrice, size: parseFloat(sizeUSDC.toFixed(2)), side },
      { tickSize: tickSize.toString(), negRisk: false }
    );

    log(`[LIVE ORDER] ${side} tokenId=${tokenId.slice(0,12)} price=${roundedPrice} size=$${sizeUSDC.toFixed(2)}`, {
    ...
    await sendTelegram('⚠️ Order failed: ' + e.message?.slice(0,80));
    return null;
  }
}
...
function isQualityMarket(m) {
  // Price filter — skip near-zero and near-certain markets
  if (m.yesPrice < CFG.minYesPrice || m.yesPrice > CFG.maxYesPrice) return false;

  // Time filter — skip markets resolving too far out
  if (m.endDate) {
  ...
  // Volume filter — needs some real liquidity
...
async function claudeAnalyzeMarkets(markets) {
  ...
  const topMarkets = qualityMarkets.length >= 5 ? qualityMarkets.slice(0, 20) : markets.slice(0, 20);
  log('Market quality filter', { total: markets.length, quality: qualityMarkets.length, using: topMarkets.length });
  ...
  const marketList = topMarkets.map((m, i) => {
    ...
    return `${i+1}. "${m.question}" mkt:${yes}%${ask}${wsPart} vol:${vol}${flowPart}`;
  }).join('\n');
  ...
  const system = `You are Claude Sonnet 4, an autonomous Polymarket trading analyst with access to REAL-TIME data.
Date: ${new Date().toUTCString()}
Bankroll: $${PERSIST.bankroll.toFixed(2)} | Win rate: ${winRate}% | Total trades: ${totalTrades}

DATA SOURCES YOU HAVE ACCESS TO:
- Live Polymarket CLOB orderbook (real buy/sell depth per market)
- WebSocket real-time prices (actual last trade price, not just midpoint)
- Recent trade flow (YES vs NO money flow, whale activity >$500)
- Live news sentiment (Alpha Vantage / NewsData)
- Crypto prices (CoinGecko live)
- Your signal history (learn from past performance)

EDGE DETECTION STRATEGIES — ranked by reliability:
1. ORDERBOOK IMBALANCE — Strong buy pressure on NO = market will drift down. Use imbalance data.
2. WHALE ACCUMULATION — Trades >$500 consistently on one side = informed money. Follow size.
3. SPREAD EXPLOITATION — Wide spread (>3%) on a near-certain market = easy edge
4. NEWS REPRICING — News exists that market hasn't priced in yet. Cite the specific news.
5. RESOLUTION LOCK — Market resolves in <30 days, price still far from 0% or 100%. Time decay.
6. LONG-SHOT BIAS — YES prices 1-6% consistently overpriced. NO has mathematical edge.
7. CORRELATION GAP — Related markets diverged. Logically they must converge.

SIGNAL QUALITY RULES:
- Minimum 6% edge with SPECIFIC data citation (not generic reasoning)
- Prioritize signals where orderbook or trade flow CONFIRMS your thesis
- Maximum 3 signals per analysis — quality over quantity
- Size: high confidence = 8% bankroll, medium = 5%, low = skip

OUTPUT FORMAT: Valid JSON only, zero markdown:
{"signals":[{"market":"exact question text","outcome":"YES or NO","confidence":"high/medium/low","edge":0.09,"size":${(PERSIST.bankroll*0.08).toFixed(2)},"reasoning":"specific data-backed reason citing orderbook/flow/news"}],"summary":"1-line market overview"}`;

  const userMsg = [
    alertBlock,
    clobContext,
    marketContext ? `NEWS & CRYPTO CONTEXT:\n${marketContext}` : '',
    `\nMARKETS TO ANALYZE (with live orderbook + trade flow):\n${marketList}`,
    memoryBlock ? `\n${memoryBlock}` : '',
    '\nAnalyze all data sources above. Return JSON signals where orderbook or flow data CONFIRMS the edge.',
  ].filter(Boolean).join('\n');
...
async function resolveOpenTrades(markets) {
  ...
  for (const trade of openTrades) {
    ...
    // A market is ONLY resolved when Polymarket closes it
    // Prices snap to exactly 0.0 or 1.0 on resolution
    // A price of 0.001 (0.1%) is still an ACTIVE open market — NOT resolved
    const yesWon = mkt.yesPrice >= 0.999;  // YES resolved: price = 1.0
    const noWon  = mkt.noPrice  >= 0.999;  // NO resolved:  price = 1.0
    ...
    // Note: prices like 0.001, 0.01, 0.009 = still OPEN, just heavily favored
    // Markets resolve on their endDate when Polymarket closes them

    // Also check endDate — if market closed, fetch actual resolution
    if (!resolved && mkt.endDate) {
      const endTs = new Date(mkt.endDate).getTime();
      if (Date.now() > endTs + 3600000) {
        // Market ended over 1 hour ago — try to get resolution from CLOB
        try {
...
async function poll() {
  ...
  try {
    // Always fetch real markets first
    const markets = await fetchMarkets();

    // Check real Polymarket resolution for open trades (uses live market data)
    await resolveOpenTrades(markets);

    // Update signal history with real outcomes
    ...
    // Start/maintain WebSocket price feed for top markets
    if (!STATE_CLOB.wsConnected && WebSocket) {
      startWebSocketPriceFeed(markets);
    }

    // Claude analysis — every poll until we get signals, then every CFG.claudeEvery polls
    const signalsSoFar = STATE.feed.filter(f => f.type === 'claude').length;
    if (pollCount === 1 || signalsSoFar === 0 || pollCount % CFG.claudeEvery === 0) {
  ...
  // Track equity curve with real PnL from resolved trades
  const curPnl = calcPnl(PERSIST.trades);
  const bankrollNow = PERSIST.bankroll + parseFloat(curPnl.toFixed(4));
  // Only push if value changed or every 10 polls (keeps chart clean)
...
async function serve(req, res) {
  ...
  // Hit this once after deploying with POLY_PRIVATE_KEY set
  // It derives and saves your apiKey, secret, passphrase automatically
  if (req.method === 'GET' && path === '/generate-creds') {
    ...
    try {
      _clobClient = null; // force reinit
      const client = await getClobClient();
      if (!client) throw new Error('Failed to init CLOB client');
      res.writeHead(200, CORS);
      res.end(JSON.stringify({
        ok: true,
        message: 'Credentials derived and saved to persistent state',
        apiKey:     (PERSIST.polyApiKey || CFG.polyApiKey || '').slice(0,8) + '...',
        hasSecret:  !!(PERSIST.polySecret || CFG.polySecret),
        walletAddr: CFG.polyWalletAddr,
        note: 'Add POLY_API_KEY, POLY_SECRET, POLY_PASSPHRASE to Railway Variables from your Railway logs',
      }));
    } catch(e) {
