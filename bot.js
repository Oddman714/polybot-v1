...
// ─── CLAUDE MARKET ANALYSIS — upgraded with proven strategies ────────────────
async function claudeAnalyzeMarkets(markets) {
  ...
  const system = `You are an expert prediction market analyst. Today: ${new Date().toUTCString()}.
Bankroll: $${PERSIST.bankroll.toFixed(2)} | Win rate: ${winRate}%

STRATEGIES THAT ACTUALLY WORK ON POLYMARKET IN 2026:
1. NEWS REPRICING: If you know recent news updates a probability but market price lags, that is edge
2. LONG-SHOT FADE: Markets at 2-8% YES often overpriced — NO side has value
3. FAVORITE-LONGSHOT BIAS: Extremes (95%+ or 2%-) are systematically mispriced — fade them
4. CORRELATION: Related markets should move together — find ones that haven't repriced yet
5. TIME DECAY: Near-resolution markets compress to 0% or 100% — find ones stuck in between

RULES:
- Only signal genuine edges >5%
- Max 3 signals
- Prefer markets with >$100K volume
- Include your reasoning in 1 sentence

Respond ONLY in JSON with no markdown:
{"signals":[{"market":"question","outcome":"YES or NO","confidence":"high/medium/low","edge":0.08,"reasoning":"why"}],"summary":"1 line overview"}`;

  try {
    const res = await httpsPost(
      'https://api.anthropic.com/v1/messages',
      { model: 'claude-sonnet-4-20250514', max_tokens: 800, system,
        messages: [{ role: 'user', content: `Markets to analyze:\n${marketList}\n\nReturn JSON signals.` }] },
    ...
    log('Claude signals', { count: signals.length, summary: parsed?.summary?.slice(0,80) });
    ...
    for (const sig of signals) {
      ...
      log(`[${CFG.dryRun?'PAPER':'LIVE'}] Claude: ${sig.outcome} "${sig.market?.slice(0,45)}" edge=${(sig.edge*100).toFixed(0)}% $${size.toFixed(2)}`);
    }

    if (signals.length > 0) {
      await sendTelegram(`🤖 *${signals.length} signal${signals.length>1?'s':''} found*\n` +
        signals.map(s => `• ${s.outcome} "${s.market?.slice(0,35)}..." +${(s.edge*100).toFixed(0)}% edge`).join('\n'));
    }
    return signals;
