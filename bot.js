...
async function callClaude(systemPrompt, userPrompt) {
  ...
  if (!apiKey || apiKey.length < 20) {
    logError('callClaude', new Error('CLAUDE_API_KEY missing or invalid — check Railway Variables'));
    return null;
  }
  log('Calling Claude API', { keyLength: apiKey.length, keyStart: apiKey.slice(0, 10) });
...
async function claudeAnalyzeMarkets(markets, headlines) {
  ...
  if (!claudeKey || claudeKey.length < 20) {
    log('Claude key not available — skipping analysis');
    return [];
  }

  // Use real markets if available, otherwise use paper markets
  let liquid = markets
    .filter(m => parseFloat(m.volume || m.volumeNum || 0) >= CFG.minVolume)
    .slice(0, 15);

  if (!liquid.length) {
    log('No real markets from API — using paper markets for Claude analysis');
    liquid = PAPER_MARKETS.map((q, i) => ({
      question: q, title: q,
...
async function poll() {
  ...
  try {
    ...
    const _ck = (CFG.claudeKey || '').replace(/[^\x20-\x7E]/g, '').trim();
    if (STATE.claudePollCount % CFG.claudePollEvery === 0 && _ck.length > 20) {
      log('Claude poll triggered', { pollCount: STATE.claudePollCount, keyLength: _ck.length });
...
app.get('/', (req, res) => {
  ...
  res.sendFile(htmlPath, (err) => {
    if (err) res.json({ ok: true, uptime: Date.now() - STATE.startedAt, version: '4.7' });
  });
});

// Health check JSON
app.get('/health', (req, res) => res.json({ ok: true, uptime: Date.now() - STATE.startedAt, version: '4.7' }));
