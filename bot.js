...
const CFG = {
  ...
  claudeKey:  (process.env.CLAUDE_API_KEY || '').replace(/[\r\n\t]/g, '').trim(),
...
app.get('/', (req, res) => {
  ...
  res.sendFile(htmlPath, (err) => {
    if (err) res.json({ ok: true, uptime: Date.now() - STATE.startedAt, version: '4.4' });
  });
});

// Health check JSON
app.get('/health', (req, res) => res.json({ ok: true, uptime: Date.now() - STATE.startedAt, version: '4.4' }));
