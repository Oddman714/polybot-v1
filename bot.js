// POLYBOT — Railway Server v10.4 — bankroll synced to $9.70 real capital
...
const CFG = {
  ...
  bankroll:    parseFloat(process.env.BANKROLL) || 9.70,
...
function loadPersist() {
  try {
    if (fs.existsSync(CFG.stateFile)) {
      ...
      // Always sync bankroll from Railway env var on startup
      // This ensures changing BANKROLL in Railway immediately takes effect
      const envBankroll = parseFloat(process.env.BANKROLL);
      if (envBankroll && envBankroll > 0) {
        // Only update if no trades have resolved yet (don't overwrite compounded bankroll)
        const hasResolvedTrades = (PERSIST.trades || []).some(t => t.status === 'win' || t.status === 'loss');
        if (!hasResolvedTrades) {
          PERSIST.bankroll = envBankroll;
          log('Bankroll synced from Railway env', { bankroll: envBankroll });
        }
      } else {
        PERSIST.bankroll = parseFloat(PERSIST.bankroll) || 9.70;
      }
    ...
    try {
      ...
      for (const trade of trades.slice(0, 5)) {
        ...
        if (!CFG.dryRun) {
          ...
          if (tokenId) {
            log(`[LIVE] Submitting CLOB order`, { market: question.slice(0,50), outcome, price, size: betSize, tokenId: tokenId.slice(0,20) });
            await executeCLOBOrder(tokenId, price, betSize, 'BUY');
          } else {
            log(`[LIVE] NO TOKEN ID - cannot submit CLOB order`, {
              market: question.slice(0,50), outcome,
              copyMktFound: !!copyMkt, mktId,
              yesTokenId: copyMkt?.yesTokenId || 'MISSING',
              noTokenId:  copyMkt?.noTokenId  || 'MISSING',
            });
            await sendTelegram(
              'COPY TRADE SKIPPED - no token ID\n' +
              question.slice(0, 60) + '\n' +
              'Market in STATE: ' + (copyMkt ? 'YES' : 'NO') + '\n' +
              'conditionId: ' + (mktId || 'none').slice(0, 30)
            );
          }
        }
      }
...
async function executeCLOBOrder(tokenId, price, sizeUSDC, side = 'BUY') {
  ...
  try {
    ...
    // ── FULL RESPONSE LOGGING — captures geo-blocks, auth errors, rejections ──
    const fullBody = JSON.stringify(result.body).slice(0, 500);
    log(`[LIVE ORDER RESPONSE] status=${result.status}`, { body: fullBody, orderId: result.body?.orderID });
    ...
    if (result.status === 200 && result.body?.orderID) {
      ...
      // ❌ FAILED — log everything so we can diagnose
      log(`[LIVE ORDER FAILED] status=${result.status} body=${fullBody}`);
      await sendTelegram(
        '❌ ORDER FAILED\n' +
        'Status: ' + result.status + '\n' +
        'Response: ' + fullBody.slice(0, 200) + '\n' +
        side + ' $' + sizeUSDC.toFixed(2) + ' @ ' + (roundedPrice*100).toFixed(1) + '%'
      );
    }
    ...
    await sendTelegram('❌ Order exception: ' + e.message?.slice(0,120));
    return null;
  }
}
...
async function serve(req, res) {
  ...
  // ── GET /test-order — fire a tiny real order to diagnose CLOB response ──
  // Uses the highest-volume market in STATE.markets with a valid tokenId
  // Sends $1 order at current yes price — reveals geo-blocks, auth errors, etc.
  if (req.method === 'GET' && path === '/test-order') {
    try {
      const mkt = STATE.markets.find(m => m.yesTokenId);
      if (!mkt) {
        res.writeHead(200, CORS);
        res.end(JSON.stringify({ ok: false, error: 'No market with tokenId found in STATE.markets' }));
        return;
      }
      log('[TEST-ORDER] Firing diagnostic order', { market: mkt.question?.slice(0,50), tokenId: mkt.yesTokenId?.slice(0,20) });
      const orderResult = await executeCLOBOrder(mkt.yesTokenId, mkt.yesPrice || 0.5, 1.0, 'BUY');
      res.writeHead(200, CORS);
      res.end(JSON.stringify({
        market: mkt.question,
        yesTokenId: mkt.yesTokenId?.slice(0,30) + '...',
        price: mkt.yesPrice,
        orderResult,
        dryRun: CFG.dryRun,
      }, null, 2));
