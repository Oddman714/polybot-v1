// POLYBOT — Railway Server v8.3 — WS fix + throttling + hardened state
...
function resolveOpenTrades() {
  ...
  for (const trade of openTrades) {
    ...
    if (won) {
      PERSIST.wins++;
      const profit = trade.size * (trade.edge || 0.08);
      addFeed({
      ...
      PERSIST.losses++;
      const lossAmt = trade.size * Math.min(trade.price || trade.edge || 0.08, 0.5);
      addFeed({
        ts: now, type: trade.type || 'claude', kind: 'model',
        market: trade.market,
        outcome: trade.outcome,
...
async function poll() {
  ...
  const curPnl = PERSIST.trades
    ...
    .reduce((s, t) => {
      if (t.status === 'win') return s + (t.size * (t.edge || 0.08));
      const riskPct = Math.min(t.price || t.edge || 0.08, 0.5);
      return s - (t.size * riskPct);
    }, 0);
...
async function serve(req, res) {
  ...
  if (req.method === 'GET' && path === '/state') {
    ...
    // Accurate P&L: win = size * edge, loss = size * price (actual risk on Polymarket)
    // Buying NO at 0.1% means max loss is 0.1% of position, not full position
    const pnl = PERSIST.trades
      .filter(t => t.status === 'win' || t.status === 'loss')
      .reduce((s, t) => {
        if (t.status === 'win') return s + (t.size * (t.edge || 0.08));
        // Loss = only what was actually at risk (price × size for low-prob markets)
        const riskPct = Math.min(t.price || t.edge || 0.08, 0.5);
        return s - (t.size * riskPct);
      }, 0);
