// POLYBOT — Railway Server v9.0 — Real resolution, no simulation — WS fix + throttling + hardened state
...
async function resolveOpenTrades(markets) {
  ...
  for (const trade of openTrades) {
    ...
    // Check if market has resolved: noPrice = 1.0 or yesPrice = 1.0
    const yesResolved = mkt.yesPrice >= 0.99;
    const noResolved  = mkt.noPrice >= 0.99 || mkt.yesPrice <= 0.01;

    let resolved = false;
    let won      = false;

    if (yesResolved) {
      // YES resolved as winner
      resolved = true;
      won = trade.outcome === 'YES';
    } else if (noResolved && mkt.yesPrice <= 0.01) {
      // NO resolved as winner (market expired or resolved NO)
      resolved = true;
