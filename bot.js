// POLYBOT — Railway Server v8.0 — CLOB + WebSocket + Real Trade Flow
...
async function fetchLeaderboard() {
  ...
  if (CFG.copyWallets.length > 0) {
    ...
    STATE.traders = CFG.copyWallets.map((addr, i) => ({
      addr, name: 'wallet-' + (i+1), pnl: 0, roi: 0, wins: 0, trades: 0,
      copying: true,
    }));
  }
}
...
function startWebSocketPriceFeed(markets) {
  ...
  try {
    ...
    ws.on('open', () => {
      ...
      // Subscribe to price change events for top markets
      ws.send(JSON.stringify({
        auth: {},
        markets: topTokenIds.map(id => ({ asset_id: id })),
        type: 'subscribe',
      }));
      ...
      try {
        ...
        for (const ev of evArr) {
          if (ev.event_type === 'price_change' || ev.event_type === 'last_trade_price') {
            const price = parseFloat(ev.price || ev.last_trade_price || 0);
    ...
    ws.on('close', () => {
      ...
      log('WebSocket disconnected — will reconnect on next market fetch');
    });
    ws.on('error', (e) => {
      STATE_CLOB.wsConnected = false;
...
function resolveOpenTrades() {
  ...
  for (const trade of openTrades) {
    ...
    const baseWinProb = 0.52 + (edge * 2); // 52% base + edge bonus
    const won = Math.random() < Math.min(baseWinProb, 0.78);
