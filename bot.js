...
function httpsGet(url, headers = {}) {
  return new Promise((resolve, reject) => {
    ...
    const opts = {
      ...
      method:   'GET',
      headers:  { 'Accept': 'application/json', 'User-Agent': 'Polybot/6.0', ...headers },
      timeout:  10000,
    };
    const req = https.request(opts, res => {
      ...
      res.on('end', () => {
        ...
        catch { reject(new Error(`JSON parse fail (status ${res.statusCode}): ${body.slice(0,200)}`)); }
      });
    });
    req.on('error', reject);
...
async function fetchMarkets() {
  const endpoints = [
    ...
    'https://gamma-api.polymarket.com/markets?closed=false&limit=50&sort=liquidity&order=desc',
  ...
  const normalized = rawMarkets
    .filter(m => {
      ...
      if (m.closed || m.archived || m.resolvedBy) return false;
...
function serve(req, res) {
  ...
  // ── GET /markets-check — debug endpoint ──
