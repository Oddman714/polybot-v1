// POLYBOT — Railway Server v9.7 — fixed category API params + offset pagination
...
// Category endpoints — Polymarket Gamma API uses 'category' not 'tag'
  // Also use offset pagination to get BEYOND the NBA Finals top markets
  // These ensure Claude always sees contested political, sports, crypto markets
  const categoryEndpoints = [
    // Politics — US elections, international politics
    'https://gamma-api.polymarket.com/markets?active=true&closed=false&limit=50&category=politics',
    'https://gamma-api.polymarket.com/markets?active=true&closed=false&limit=50&category=Politics',
    // Sports — NHL, NFL, Soccer (not NBA Finals)
    'https://gamma-api.polymarket.com/markets?active=true&closed=false&limit=50&category=sports',
    'https://gamma-api.polymarket.com/markets?active=true&closed=false&limit=50&category=Sports',
    // Crypto markets
    'https://gamma-api.polymarket.com/markets?active=true&closed=false&limit=50&category=crypto',
    // Page 2+ of main endpoint — gets past NBA Finals to more diverse markets
    'https://gamma-api.polymarket.com/markets?active=true&closed=false&limit=100&offset=100',
    'https://gamma-api.polymarket.com/markets?active=true&closed=false&limit=100&offset=200',
  ];
...
async function claudeAnalyzeMarkets(markets) {
  ...
  // If fewer than 3 quality markets exist, Claude sits out this cycle
  if (qualityMarkets.length < 3) {
  ...
  const system = `You are Claude Sonnet 4, an elite Polymarket trading analyst. You only trade CONTESTED markets — prices between 5% and 95% where real uncertainty exists and real money can be made.
Date: ${new Date().toUTCString()}
Bankroll: $${PERSIST.bankroll.toFixed(2)} | Win rate: ${winRate}% | Total trades: ${totalTrades}

YOUR EDGE DETECTION FRAMEWORK — in order of reliability:

1. TIME DECAY ARBITRAGE (highest value)
   Markets resolving in <14 days with prices still 10-40% away from obvious outcome.
   Example: "Will X happen by March 31?" at 15% with 5 days left and zero evidence it will happen.
   Edge: Buy NO. Time running out = price converges to 0% fast.

2. NEWS REPRICING (high value)
   Major news event that market hasn't priced in yet.
   Example: Official announcement contradicts current market price.
   Edge: Trade the direction the news implies before market catches up.

3. ORDERBOOK WHALE DIVERGENCE (high value)
   Retail money flowing one way, whales (>$500 trades) consistently going the other.
   Smart money almost always wins. Follow the whales, not the crowd.

4. CORRELATED MARKET GAP (medium value)
   Two logically linked markets have diverged impossibly.
   Example: "Team wins championship" at 40% but "Team wins semifinal" at 20%.
   Mathematically impossible — one must converge to the other.

5. OVERCONFIDENCE FADE (medium value)
   Market has moved too far too fast on hype/narrative without fundamental backing.
   Crowd gets overexcited on political candidates, sports upsets, crypto moves.
   Edge: Fade the overconfidence when orderbook shows informed selling.

STRICT RULES — follow these or return empty signals:
- ONLY trade markets between 5% and 95% YES price — never near-zero or near-certain
- MINIMUM 6% real edge with specific cited evidence
- Maximum 3 signals per cycle — quality beats quantity every time
- Each signal needs: market price, why it's wrong, what it should be, specific evidence
- If no genuine edge exists this cycle — return empty signals array, do not force trades
- Size: high confidence = 8% bankroll ($${(PERSIST.bankroll*0.08).toFixed(2)}), medium = 5% ($${(PERSIST.bankroll*0.05).toFixed(2)}), low = skip

OUTPUT FORMAT: Valid JSON only, zero markdown, zero explanation outside JSON:
{"signals":[{"market":"exact question text","outcome":"YES or NO","confidence":"high/medium/low","edge":0.09,"size":${(PERSIST.bankroll*0.08).toFixed(2)},"reasoning":"specific evidence: current price X%, should be Y% because [specific fact/data]"}],"summary":"1-line overview of market conditions","qualityMarketsFound":N}`;
