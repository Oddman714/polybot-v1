...
async function processTrade(trader, pos) {
  ...
  const traderSeen = seen.get(trader.address);

  // Cycle 1: build baseline silently, no signals
  if (!baselined) { traderSeen.add(pos.marketId); return; }

  // Already seen this position
  if (traderSeen.has(pos.marketId)) return;
  traderSeen.add(pos.marketId);

  const size = Math.min(CFG.MAX_TRADE, pos.value * CFG.COPY_RATIO);
  if (size < 1) { log('skip', `Skip - too small ($${size.toFixed(2)})`); return; }
...
async function runCycle() {
  ...
  if (traders.length === 0) {
    log('retry', 'Trader list empty - retrying...');
    const fresh = await fetchTopTraders();
    if (fresh.length) traders = fresh;
  }

  STATE.cycles = cycleCount;
  log('cycle', `Cycle #${cycleCount} - watching ${traders.length} wallets`);

  for (const trader of traders) {
    const positions = await getPositions(trader.address);
    for (const pos of positions) await processTrade(trader, pos);
  }

  if (!baselined) {
    baselined = true;
    log('ready', `Baseline set - watching for NEW positions across ${traders.length} wallets`);
    await tg(`<b>Polybot baseline ready</b>\nWatching ${traders.length} top traders for new entries.`);
  }
}
...
async function main() {
  ...
  console.log('╚══════════════════════════════════════════════╝');
  console.log('');
  console.log(`  Mode:   ${CFG.DRY_RUN ? '✏️  PREVIEW (paper — no real trades)' : '⚡ LIVE (real USDC)'}`);
