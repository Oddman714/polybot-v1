...
const CFG = {
  ...
  claudeKey:  (process.env.CLAUDE_API_KEY || '').split('').filter(c => c.charCodeAt(0) >= 32 && c.charCodeAt(0) <= 126).join('').trim(),
...
async function callClaude(systemPrompt, userPrompt) {
  ...
  return new Promise((resolve, reject) => {
    const opts = {
      ...
      headers:  {
        'Content-Type':      'application/json',
        'x-api-key':         CFG.claudeKey.replace(/[\r\n\t]/g, '').trim(),
...
app.get('/', (req, res) => {
  ...
  res.sendFile(htmlPath, (err) => {
    if (err) res.json({ ok: true, uptime: Date.now() - STATE.startedAt, version: '4.5' });
  });
});

// Health check JSON
app.get('/health', (req, res) => res.json({ ok: true, uptime: Date.now() - STATE.startedAt, version: '4.5' }));
