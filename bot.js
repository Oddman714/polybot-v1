...
// Returns { apiKey, secret, passphrase } or null
async function deriveClobCreds() {
  ...
  try {
    ...
    if (result.status !== 200) {
      logError('deriveClobCreds', `HTTP ${result.status}: ${JSON.stringify(result.body)}`);
      return null;
    }

    const creds = result.body;
    log('CLOB credentials derived', { key: (creds.apiKey||'').slice(0,8) + '...' });
    return {
...
async function serve(req, res) {
  ...
  if (req.method === 'GET' && path === '/generate-creds') {
    ...
    try {
      ...
      if (!creds) throw new Error('Credential derivation returned null — check Railway logs');
