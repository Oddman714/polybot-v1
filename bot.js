...
async function fetchClobPrices(conditionId) {
  // Fetch live YES and NO token prices from Polymarket CLOB
  try {
    ...
    const yesTokenId = tokens.find(t => t.outcome === 'Yes')?.token_id || tokens[0]?.token_id;
    const noTokenId  = tokens.find(t => t.outcome === 'No')?.token_id  || tokens[1]?.token_id;
    if (!yesTokenId || !noTokenId) return null;

    // Fetch best ask for both YES and NO tokens simultaneously
    ...
    if (isNaN(yesAsk) || isNaN(noAsk)) return null;
...
async function scanArb(markets) {
  ...
  const candidates = markets
    .filter(m => parseFloat(m.volume || m.volumeNum || 0) > 10000)
    .slice(0, 8);
...
app.get('/', (req, res) => {
  ...
  res.sendFile(htmlPath, (err) => {
    if (err) res.json({ ok: true, uptime: Date.now() - STATE.startedAt, version: '5.1' });
  });
});

// Health check JSON
app.get('/health', (req, res) => res.json({ ok: true, uptime: Date.now() - STATE.startedAt, version: '5.1' }));
